      ...
      {({ values, errors, touched, handleBlur, handleChange, handleSubmit, resetForm }) => (
        <form onSubmit={handleSubmit} className='flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8'>
          <div className='text-sm sm:mx-auto sm:w-full sm:max-w-sm'>
            <h1 className='text-center text-2xl font-semibold text-title'>{isLogin ? '로그인' : '회원가입'}</h1>
            ...
            <div className='mt-10 grid gap-6 text-title'>
              ...
              <button type='submit' className='bg-accent py-3 font-semibold text-white hover:bg-accentHover'>
                {isLogin ? '로그인' : '회원가입'}
              </button>
              <p className='cursor-pointer text-end text-other'>
                {isLogin ? '아직 계정이 없으신가요?' : '이미 계정이 있으신가요?'}
                <span className='px-1'>&gt;</span>
                <Link to={isLogin ? '/register' : '/login'} className='font-semibold text-accent'>
                  {isLogin ? '회원가입' : '로그인'}
                </Link>
              </p>
            </div>
